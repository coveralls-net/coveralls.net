...
namespace Coveralls.Tests
{
    public class AutoParserTests
    {
        ...
        {
            var parser = new AutoParser() { Report = TestHelpers.LoadResourceXml("coveralls.tests.netcore.Files.OpenCover.SingleFileCoverage.xml") };

            var coverageFile = parser.Generate().First();

            coverageFile.Source = TestHelpers.LoadResourceText("coveralls.tests.netcore.Files.Utilities.cs");
        ...
        {
            var parser = new AutoParser() { Report = TestHelpers.LoadResourceXml("coveralls.tests.netcore.Files.Cobertura.SingleFileCoverage.xml") };

            var coverageFile = parser.Generate().First();

            coverageFile.Source = TestHelpers.LoadResourceText("coveralls.tests.netcore.Files.app.ts");
