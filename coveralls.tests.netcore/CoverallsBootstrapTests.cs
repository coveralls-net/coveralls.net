...
namespace Coveralls.Tests
{
    ...
    public class CoverallsBootstrapTests
    {
        ...
        {
            var fileSystem = Substitute.For<IFileSystem>();
            fileSystem.ReadFileText("").ReturnsForAnyArgs(TestHelpers.LoadResourceText("coveralls.tests.netcore.Files.OpenCover.SingleFileCoverage.xml"));
        ...
        {
            var fileContents = TestHelpers.LoadResourceText("coveralls.tests.netcore.Files.Utilities.cs");
            var fileContentsUnix = fileContents.Replace("\r\n", "\n");

            var fileSystem = Substitute.For<IFileSystem>();
            fileSystem.ReadFileText("coverage.xml").Returns(TestHelpers.LoadResourceText("coveralls.tests.netcore.Files.OpenCover.SingleFileCoverage.xml"));
